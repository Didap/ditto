  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-1">
        <a href="/dashboard" className="text-sm text-(--ditto-text-muted) hover:text-(--ditto-text)">← Library</a>
      </div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold tracking-tight text-(--ditto-text)">Genera un Design Ispirato</h1>
        <p className="text-sm text-(--ditto-text-muted) mt-1">Passa le tue ispirazioni, rispondi a qualche domanda, Ditto le fonde nel tuo stile</p>
      </div>

      {/* ═══ URL INPUT ═══ */}
      {phase === "urls" && (
        <div className="max-w-2xl">
          <h2 className="text-lg font-semibold text-(--ditto-text) mb-4">Scegli le tue ispirazioni</h2>
          <p className="text-sm text-(--ditto-text-muted) mb-6">Inserisci 2-10 URL di siti che ti piacciono. Ditto assorbirà il design di ognuno.</p>
          <div className="flex flex-col gap-3">
            {urls.map((url, i) => (
              <div key={i} className="flex gap-2 items-center">
                <span className="text-sm text-(--ditto-text-muted) w-6">{i + 1}.</span>
                <input
                  type="url"
                  value={url}
                ...
                />
                {urls.length > 2 && (
                  <button onClick={() => setUrls(urls.filter((_, j) => j !== i))} className="text-(--ditto-text-muted) hover:text-(--ditto-error) text-sm px-2"><X className="w-4 h-4" /></button>
                )}
              </div>
            ))}
          </div>
          {urls.length < 10 && (
            <button onClick={() => setUrls([...urls, ""])} className="mt-3 text-sm text-(--ditto-primary) hover:text-(--ditto-primary-hover)">+ Aggiungi ispirazione</button>
          )}
          ...
          <div className="mt-6 rounded-xl border border-(--ditto-border) bg-(--ditto-surface) p-4">
            ...
            >
              <div>
                <h3 className="text-sm font-semibold text-(--ditto-text)">Oppure scegli dalla tua libreria</h3>
              ...
              <span className="text-(--ditto-text-muted) text-sm">{showCatalog ? "▲" : "▼"}</span>
            </button>
          ...
          <div className="mt-6 mb-2">
            <h3 className="text-sm font-semibold text-(--ditto-text) mb-3">Scegli la modalità</h3>
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                <div className="flex items-center gap-2 mb-2">
                  <Zap className="w-5 h-5 text-(--ditto-primary)" strokeWidth={1.5} />
                  <span className="text-base font-semibold text-(--ditto-text) group-hover:text-(--ditto-primary)">Auto</span>
                </div>
              ...
              >
                <div className="flex items-center gap-2 mb-2">
                  <Crosshair className="w-5 h-5 text-(--ditto-primary)" strokeWidth={1.5} />
                  <span className="text-base font-semibold text-(--ditto-text) group-hover:text-(--ditto-primary)">Precisa</span>
                </div>
                <p className="text-xs text-(--ditto-text-muted) leading-relaxed">
        ...
        <div>
          ...
          {mode === "precisa" && !currentReady && !questionnaireDone && (
            <div className="flex flex-col items-center py-20">
              ...
              <p className="text-lg font-semibold text-(--ditto-text)">Ditto sta assorbendo il primo design...</p>
              <p className="text-sm text-(--ditto-text-muted) mt-1">Appena pronto, iniziamo con le domande</p>
            </div>
          )}
          ...
          {mode === "auto" && extractingCount > 0 && (
            <div className="flex flex-col items-center py-20">
              <span className="inline-block mb-4"><LottieLoader size={200} /></span>
              <p className="text-lg font-semibold text-(--ditto-text)">Ditto sta analizzando automaticamente...</p>
              <p className="text-sm text-(--ditto-text-muted) mt-1">{readyCount}/{inspirations.length} ispirazioni analizzate</p>
            </div>
          )}
          ...
          {mode === "auto" && extractingCount === 0 && readyCount > 0 && !generating && !result && (
            <div>
              <div className="mb-6">
                <h2 className="text-lg font-semibold text-(--ditto-text) mb-1">Analisi Automatica Completata</h2>
                <p className="text-sm text-(--ditto-text-muted)">Ecco cosa Ditto ha rilevato da ogni ispirazione. Verifica le risposte prima di generare.</p>
              </div>
              ...
              <div className="space-y-4 mb-8">
                {inspirations.filter((insp) => insp.status === "ready").map((insp) => {
                  ...
                  return (
                    <div key={realIdx} className="rounded-xl border border-(--ditto-border) bg-(--ditto-surface) overflow-hidden">
                      {/* Inspiration header */}
                      <div className="flex items-center gap-3 px-4 py-3 border-b border-(--ditto-border)">
                        {insp.screenshot && (
                          // eslint-disable-next-line @next/next/no-img-element
                          <img src={insp.screenshot} alt={insp.name} className="w-10 h-10 rounded-md object-cover border border-(--ditto-border)" />
                        )}
                        <div className="flex-1 min-w-0">
                          <h3 className="text-sm font-semibold text-(--ditto-text) truncate">{insp.name || insp.url}</h3>
                          <p className="text-[11px] text-(--ditto-text-muted) truncate">{insp.url}</p>
                        </div>
                        {insp.resolved && (
                          <div className="flex gap-1 shrink-0">
                            {[insp.resolved.colorPrimary, insp.resolved.colorSecondary, insp.resolved.colorAccent].map((c, ci) => (
                              <div key={ci} className="w-5 h-5 rounded border border-(--ditto-border)" style={{ backgroundColor: c }} />
                            ))}
                          </div>
                        )}
                      </div>
                        ...
                        {answers.map((answer) => (
                          <div key={answer.questionId} className="px-3 py-3">
                            <div className="flex items-center gap-1.5 mb-1">
                              <MoodIcon name={answer.chosenIcon} className="w-4 h-4 text-(--ditto-primary)" />
                              <span className="text-xs font-semibold text-(--ditto-text)">{answer.chosenOption}</span>
                            </div>
                            <p className="text-[10px] text-(--ditto-text-muted) leading-snug mb-1.5">{answer.chosenDescription}</p>
                            <div className="flex items-center gap-1">
                              <span className={`inline-block w-1.5 h-1.5 rounded-full ${
                                answer.confidence === "high" ? "bg-green-500" :
                                answer.confidence === "medium" ? "bg-yellow-500" : "bg-red-400"
                              }`} />
                              <span className="text-[9px] text-(--ditto-text-muted)">{answer.reason}</span>
                            </div>
                          </div>
                        ))}
                      </div>
                  ...
                  <div className="rounded-lg border border-(--ditto-border) bg-(--ditto-surface) p-4">
                    ...
                    {currentInsp.resolved && (
                      <div className="mt-3 text-xs text-(--ditto-text-muted)">
                        <span className="font-medium text-(--ditto-text-secondary)">Font:</span>{" "}
              ...
              <div className="col-span-3">
                ...
                <div className="flex flex-col gap-3">
                  {MOOD_QUESTIONS[qIdx].options.map((option, i) => (
                    ...
                    >
                      <MoodIcon name={option.icon} className="w-6 h-6 text-(--ditto-primary)" />
                      <div>
          ...
          {questionnaireDone && extractingCount === 0 && readyCount > 0 && (
            <div>
              <h2 className="text-lg font-semibold text-(--ditto-text) mb-2">Mappa delle Ispirazioni</h2>
              ...
              <div className="relative w-full h-72 rounded-xl border border-(--ditto-border) bg-(--ditto-surface) mb-6 overflow-hidden">
                <div className="absolute inset-0 flex items-center justify-center"><div className="w-px h-full bg-(--ditto-border) opacity-50" /></div>
                <div className="absolute inset-0 flex items-center"><div className="h-px w-full bg-(--ditto-border) opacity-50" /></div>
                <span className="absolute top-2 left-1/2 -translate-x-1/2 text-[10px] text-(--ditto-text-muted)">Serio</span>
                <span className="absolute bottom-2 left-1/2 -translate-x-1/2 text-[10px] text-(--ditto-text-muted)">Giocoso</span>
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[10px] text-(--ditto-text-muted)">Calmo</span>
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-(--ditto-text-muted)">Audace</span>
                ...
                {inspirations.filter((i) => i.status === "ready").map((insp, idx) => {
                  ...
                  return (
                    <div key={idx} className="absolute -translate-x-1/2 -translate-y-1/2 flex flex-col items-center" style={{ left: `${x * 80 + 10}%`, top: `${y * 80 + 10}%` }}>
                      ...
                      <span className="text-[10px] text-(--ditto-text-secondary) mt-1 bg-(--ditto-surface) px-1 rounded whitespace-nowrap max-w-[80px] truncate">{insp.name}</span>
                    </div>
                  );
                })}
              </div>
                ...
                {inspirations.filter((ins) => ins.status === "ready").map((insp) => {
                  ...
                  return (
                    <div key={realIdx} className="flex items-center gap-3">
                      ...
                      <span className="text-sm text-(--ditto-text-secondary) w-28 truncate">{insp.name}</span>
                      ...
                      />
                      <span className="text-sm text-(--ditto-text-muted) w-10 text-right">{Math.round(insp.weight * 100)}%</span>
                    </div>
                  );
                })}
              </div>
              ...
              <div className="rounded-lg border border-(--ditto-border) bg-(--ditto-surface) p-4 mb-6">
                <h3 className="text-sm font-semibold text-(--ditto-text) mb-3">Profilo</h3>
                  ...
                  {MOOD_DIMENSIONS.map((dim) => {
                    ...
                    return (
                      <div key={dim.id} className="flex items-center gap-2">
                        <span className="text-[11px] text-(--ditto-text-muted) w-20 shrink-0">{dim.label}</span>
                        <div className="flex-1 h-1.5 bg-(--ditto-bg) rounded-full overflow-hidden">
                          <div className="h-full rounded-full bg-(--ditto-primary) transition-all" style={{ width: `${(avg + 1) / 2 * 100}%` }} />
                        </div>
                        <span className="text-[10px] text-(--ditto-text-muted) w-20 text-right">{avg < -0.3 ? dim.poles[0] : avg > 0.3 ? dim.poles[1] : "Bilanciato"}</span>
                      </div>
                    );
                  })}
                </div>
              ...
              {!canGenerate && credits !== null && (
                <div className="rounded-lg border border-(--ditto-warning)/30 bg-(--ditto-warning)/10 px-4 py-2.5 mb-3">
                  <p className="text-sm text-(--ditto-warning)">Crediti insufficienti. Servono 300 crediti, ne hai {credits}.</p>
                </div>
              )}
      ...
      {phase === "result" && editResolved && result && (
        <div>
          ...
          {showSaveModal && saveState !== "saved" && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
              <div className="w-full max-w-md rounded-xl border border-(--ditto-border) bg-(--ditto-surface) p-6 shadow-2xl">
                ...
                <div className="space-y-3 mb-6">
                  <div>
                    <label className="block text-sm font-medium text-(--ditto-text) mb-1">Nome</label>
                    <input
                      type="text"
                      value={saveName}
                  ...
                  <div>
                    <label className="block text-sm font-medium text-(--ditto-text) mb-1">
                      Descrizione <span className="font-normal text-(--ditto-text-muted)">(opzionale)</span>
                    </label>
                    <input
                      type="text"
          ...
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg font-semibold text-(--ditto-text)">Il tuo Design Ibrido</h2>
            <div className="flex items-center gap-3">
              {saveState === "saved" && savedSlug ? (
                <a href={`/design/${savedSlug}`} className="text-sm text-(--ditto-primary) hover:text-(--ditto-primary-hover)">
                  Apri nel dettaglio →
                </a>
              ) : saveState !== "saving" && (
                <button
